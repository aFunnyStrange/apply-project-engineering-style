...
先区分客户端与高度封装、高并发的服务端：后者可以保留完整分层。客户端优先在 api 中
组装完整请求对象、调用链直接的风格。分层表示职责，不要求客户端凑齐 handlers、services、
repositories。协议输入输出与平台业务状态分开，减少纯转发，通过参数注入已有宿主能力，轮询节奏由
worker 统一负责。详见[轻量平台包参考](references/lightweight-platform-packages.md)。

已有项目默认最小侵入：保留宿主服务端结构，客户端封装为独立包，提供 API 及必要的 runtime 能力，
只改必要的接入位置。除非明确要求大型重构，不因接入一个客户端而重排总项目架构。

子包默认直接复用宿主传入的连接、连接池或客户端，不重复建设 infra。独立运行时才由包内入口或
runtime 创建自有资源，并只关闭自有资源。包内 settings 保留完整配置入口，公共字段允许明确引用或
映射宿主配置；这是合理复用，不属于多余业务转发。已传入资源优先，不再按配置重复实例化。
...
- 需要账号锁时，区分权威事实与 TTL 协调，保留项目选定的存储并说明持久性、保留与丢失语义；
...
[嵌入式工作流参考](references/embedded-workflows.md) 总结源码包与发行包的区别、连接与刷新回调注入、
改名时保持持久化标识兼容、按阶段区分重试和异步续查、原子容量补充、正式与调试策略隔离，以及
独立子包的架构文档。规则不携带平台专用阈值、代理配置，也不隐含授权迁移存储或开启上线开关。


## 多平台聚合应用

[多平台聚合参考](references/multi-platform-aggregation.md) 说明公共能力与总业务入口分离、纯协议包与上层编排的边界、
平台独立配置与 runtime、过渡响应 adapters、通用重试机制与平台决策、总服务配置快照，以及仅使用响应模型或规则匹配的轻量接入。
仅调整指定平台和公共边界；计费、账号策略和具体响应状态仍由各项目决定。历史响应验证应只读，并区分本地模板修改与线上配置部署。


## 编码过程中可以调整规则

使用 Skill 编码期间，你可以随时补充或修正规则，无须先修改 Skill 文件。明确的用户要求在本次任务授权范围内优先于默认规范；
发现有证据支持的缺陷或不适用的默认设计时，应调整并验证，不能为了形式合规保留问题。项目专用例外保留在项目中；
当你要求更新 Skill 时，再将可复用的经验整理为通用规则。


## 可直接阅读的客户端组件

接口构造函数直接展示完整 URL、方法、请求头、查询与请求体。业务计算字段独立封装，标准库/通用加密
原语和传输放在底层；parser 可以接收离线响应单独使用。这些能力通过明确的公共入口导出，不为静态
字典额外增加参数层。[客户端组件参考](references/client-components.md) 提供完整的虚构示例与验收标准，
Skill 不写入专项项目名称、开发机路径或专有协议默认值。

对于框架型爬虫，直接复用框架原生 Request。独立 API 模块平铺完整接口字段，算法和解析放在
spider 文件之外；spider 只保留业务顺序、回调和下一步请求编排。
